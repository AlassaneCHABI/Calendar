...
function openModal_add_even(dateStr) {
   ...
   // Convertir la date en format lisible
    ...
    const modalHTML = `
    <div class="modal fade modal-fullscreen-sm-down" id="addEventModal" tabindex="-1" aria-labelledby="addEventModalLabel" aria-hidden="true">
            ...
            <form id="add-event-form" enctype="multipart/form-data">
             ...
             <div class="separateur"></div>
              ...
              <div class="mb-3">
                ...
                <p id="selected-address" style="margin: 0;text-align: left;"></p> <!-- Résultat de l'adresse -->
                <button type="button" id="remove-address-button" style="display: none; margin-left: 10px; background: none; border: none; cursor: pointer; font-size: 13px;color: red;" onclick="removeAddress()">
...
function getStatusText(status) {
  ...
  // modal notification
  function openModal_notification() {
    // Initialiser et afficher le modal
    ...
    fetch(`${php_vars.ajax_url}?action=make_read_all_seen`, {
        method: 'GET'
    })
    .then(response => response.json())
    .then(data => {
...
function hideModal_notification() {
    // Initialiser et afficher le modal
    var myModal = bootstrap.Modal.getInstance(document.getElementById('notificationModal'));
    myModal.hide(); 
}
